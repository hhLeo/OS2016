...
older sibling:    proc->optr    (proc is younger sibling/sister)
...
struct proc_struct *myproc = NULL;
// current proc
...
alloc_proc(void) {
     ...
     */
        ...
        proc->pid = -1;
        cprintf("now the proc_state is PROC_UNINIT, proc_pid is %d in alloc_proc\n", proc->pid);
        proc->cr3 = boot_cr3;
    }
    return proc;
}
...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        cprintf("now in proc_run, the proc pid is %d \n", proc->pid);
    }
}
...
repeat:
	...
	while ((le = list_next(le)) != list) {
		...
		if (proc != NULL) {
		    ...
		    if (proc->state == PROC_ZOMBIE) {
                cprintf("proc->state is PROC_ZOMBIE, the proc pid is %d ,then do found\n", proc->pid);
			     goto found;
		   }
		}
	}
    if (haskid) {
        ...
        cprintf("current state is PROC_SLEEPING in do_wait, and the pid is %d \n", proc->pid);
        schedule();
        goto repeat;
    }
    return -E_BAD_PROC;
...
do_exit(int error_code) {
    ...
    cprintf(" now go to PROC_ZOMBIE proc pid %d in exit\n", current->pid);
	bool intr_flag;
    struct proc_struct *proc;
    local_intr_save(intr_flag);
    {
...
proc_init(void) {
    ...
    cprintf("in proc_init : idleproc 's state is PROC_RUNNABLE\n");
    ...
    int mypid = kernel_thread(init_main, "init main3 : Hello world!!", 0);
    if (pid1 <= 0 || pid2<=0 ||mypid <= 0) {
        panic("create kernel thread init_main1 or 2 or myproc failed.\n");
    }

    initproc1 = find_proc(pid1);
	initproc2 = find_proc(pid2);
    myproc = find_proc(mypid);
    set_proc_name(initproc1, "init1");
	set_proc_name(initproc2, "init2");
    set_proc_name(myproc, "myproc:init3");
