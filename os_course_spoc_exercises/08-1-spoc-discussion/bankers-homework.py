...
class Bankers(object):
    ...
    def ExecuteProcess(self,index):
        ...
        # YOUR CODE, cai wen jing 2013011425
        for j in range(0, len(self.need[index])):
            if self.avaliable[j] < self.need[index][j]:
                return False;
        ...
        #self.avaliable = self.Difference(self.avaliable, self.need[index]);
        for j in range(0, len(self.need[index])):
            self.avaliable[j] -= self.need[index][j];
            self.allocated[index][j] += self.need[index][j];
            self.need[index][j] = 0; # need = request
        #allocating END here
        return True;
        pass
        ...
        for arr in permArray:
            for i in arr:
                if self.finished[i] == False:
                    print "Executing..."
                    print "Request: "
                    print self.need[i]  # request is just the need[i]
                    #check if less avaliable than Request
                    if self.ExecuteProcess(i):    #if avaliable >= request then suppose alloc this request to the proc and check if is safe
