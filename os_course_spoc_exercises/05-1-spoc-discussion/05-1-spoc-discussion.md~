...
- 程序和进程联系和区别是什么？
```
>进程是指具有一定功能的程序在一个数据集合上的动态执行过程。
程序是有序代码的集合。
>（1）联系：
进程是操作系统处于运行状态程序的抽象；
同一个程序的多次执行过程对应为不同进程；
进程执行需要资源（内存用来保存代码和数据，CPU用来执行指令）；
（2）区别:
进程是动态的，程序是静态的（进程是程序的执行，进程有核心态/用户态）；
进程是短暂的，程序是永久的（进程是一个状态变化的过程，程序可永久保存）；
...
```
--进程控制块（PCB）是操作系统管理控制进程运行所用的信息集合；功能是：
操作系统用PCB来描述进程的基本情况以及运行变化的过程；PCB是进程存在的唯一标志
--进程控制块包含进程标识信息，处理机现场保存和进程控制信息。
```
...
```
--进程的生命周期有：
进程创建（系统初始化or用户请求创建新进程or系统调用，创建结束进入就绪状态），进程执行（处理机选择一个就绪进程，让他占用处理机并执行），进程等待（进程等待另一种服务的到来），进程抢占（高优先级进程就绪or当前进程执行时间用完，进程被抢占，进入就绪状态），进程唤醒（等待结束，进入就绪状态），进程结束（退出，让出资源）。
```
