...
# cmd:
# python working-set-policy.py -c
# python working-set-policy.py -a 5,4,1,3,3,4,2,3,5,3,5,1,4 -p WS -f 4 -c
# -a: addresses, a set of comma-separated pages to access
# -p: policy
# -f: window
...
#
parser = OptionParser()
parser.add_option('-a', '--addresses', default='5,4,1,3,3,4,2,3,5,3,5,1,4',   help='a set of comma-separated pages to access; -1 means randomly generate',  action='store', type='string', dest='addresses')
parser.add_option('-p', '--policy', default='WS',    help='replacement policy: FIFO, LRU, OPT, CLOCK, WS',                action='store', type='string', dest='policy')
...
parser.add_option('-f', '--pageframesize', default='4',    help='size of the physical page frame, in pages',                                  action='store', type='string', dest='pageframesize')
...
if options.solve == False:
    ...
    if policy == 'FIFO':
        ...
        leftStr = 'LRU'
        riteStr = 'MRU'
    elif policy == 'OPT' or  policy == 'CLOCK' or policy == 'WS':
        leftStr = 'Left '
        riteStr = 'Right'
    else:
    ...
    # window content for working set policy
    window = []
