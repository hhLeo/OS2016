...
class block:
    ...
    def addTarget(self, target):  #add for softlink
        assert(self.ftype == 's')
        self.target = target

    def getTarget(self):    # add for softlink
        assert(self.ftype == 's')
        return self.target
...
class inode:
    ...
    def setAll(self, ftype, addr, refCnt):
        assert(ftype == 'd' or ftype == 'f' or ftype == 'free' or ftype == 's') #softlink
        self.ftype  = ftype
        self.addr   = addr
        self.refCnt = refCnt
...
class fs:
    ...
    # YOUR CODE, cai wenjing 2013011425
    ...
    # DONE
        temp_inode = self.inodes[inum];
        if(temp_inode.refCnt == 1):
            self.dataFree(temp_inode.getAddr())
            self.inodeFree(inum)  
            self.nameToInum.pop(tfile)  
        else:
            temp_inode.decRefCnt() 
        ...
        parent_addr = parent_inode.getAddr()
        parent_data = self.data[parent_addr]
        parent_data.delDirEntry(tfile);
    ...
    # DONE
        ...
        assert(parent_data.dirUsed < parent_data.maxUsed)
        if(parent_data.dirEntryExists(newfile)):
            return -1;
        tinum = self.nameToInum[target]
        parent_inode.incRefCnt();
        self.inodes[tinum].incRefCnt();
        parent_data.addDirEntry(newfile, tinum);
        return tinum

    def createSoftlink(self, target, newfile, parent):
        #2013011425 
        # it is just like creating a new special file, which is [s a:dnum 1] as inode and [target] as datablock
        parent_inum = self.nameToInum[parent]
        parent_inode = self.inodes[parent_inum]
        parent_data = self.data[parent_inode.getAddr()]
        assert(parent_data.dirUsed < parent_data.maxUsed)
        ...
        self.data[dnum].addTarget(target)
        parent_inode.incRefCnt()
    ...
    # DONE
        ...
        parent_inode = self.inodes[parent_inum]
        parent_data = self.data[parent_inode.getAddr()]
        if(parent_data.dirUsed >= parent_data.maxUsed):
            return -1;
        if(parent_data.dirEntryExists(newfile)):#just make sure it is unique in the parent is ok
            return -1;
        new_inum = self.inodeAlloc()#return num
        if(new_inum < 0):
            return -1
        new_inode = self.inodes[new_inum]
        new_inode.setType(ftype)
        if(ftype == 'f'):
            new_inode.setAll('f' , -1 , 1)
        else:
            new_dNum = self.dataAlloc()
            new_data = self.data[new_dNum]
            new_data.setType('d')
            new_data.addDirEntry('.' , new_inum)
            new_data.addDirEntry('..' , self.nameToInum[parent])
            new_inode.setAll(ftype, new_dNum, 2)    
        parent_inode.incRefCnt()
        parent_data.addDirEntry(newfile , new_inum)
        inum = new_inum
        return inum
    ...
    # DONE
        if curSize == 0:
            ...
            if new_dNum == -1:
                return -1
            self.inodes[inum].setAddr(new_dNum)
            new_data.setType('f')
            new_data.addData(data)
        else:
            # write file data
            temp_data = self.data[self.inodes[inum].getAddr()]
            temp_data.addData(data)
        if printOps:
