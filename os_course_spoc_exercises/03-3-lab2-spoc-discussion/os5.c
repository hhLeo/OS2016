...
// char pg_mem[19 * 4096];
// page dir + 16 kernel entries + 1 user entry + alignment
setup_user_paging()
{
...
setup_kernel_paging()
{
  //YOUR CODE: lec7-spoc challenge-part1
	  ...
	  pg_dir = (int *)((((int)&pg_mem) + 4095) & -4096);	// for alignment

	  // ** set the 1-level page table **
	  // 16 kernel entries
	  pg_tbl[0] = pg_dir +1024;	// first page
	  for (i=1;i<16;i++)
		  pg_tbl[i] = pg_tbl[i-1] + 1024;

	  // KERSTART= 0xc0000000, // start of kerne address space
	  // mapping (KERSTART >> 12 >> 10 = 12 << 6 = 768)
	  // 768 + 16 = 784
	  for (i=0;i<1024;i++) {
		  if (768 <= i && i < 784)
			  pg_dir[i] = (int)pg_tbl[i - 768] | PTE_P | PTE_W;  // identity map 16M
		  else
			  pg_dir[i] = 0;
	  }
	  ...
	  for (i=0;i<16;i++)
		  for (j=0;j<1024;j++)
			  pg_tbl[i][j] = ((1024*i+j)<<12) | PTE_P | PTE_W;  // trick to write all 4 contiguous pages
